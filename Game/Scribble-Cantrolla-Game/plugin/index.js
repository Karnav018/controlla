/**
 * Scribble — draw & guess, as a Controlla platform plugin.
 *
 * Game logic ported from ../server (Dharmik's standalone Scribble game):
 * same word banks, hint reveals, and scoring (500 base − 2/s elapsed +
 * placement bonus, min 10; drawer +100 per correct guess). The platform owns
 * players, phones, and screens: the drawer's phone becomes a canvas, the
 * guessers' phones a guess box, and the TV renders the drawing from host
 * state. No sockets, no timers of our own, no Math.random — everything goes
 * through the plugin context (see docs/GAME_PROVIDER_GUIDE.md).
 */
...
export function createPlugin() {
  ...
  // ── hint logic (ported from gameManager.generateHint) ─────────────────
  ...
  // Ported from gameHandler.isCloseGuess: same length, exactly 1 letter off.
  ...
  async function startTurn() {
    ...
    // Advance to the next drawer still in the room.
    for (;;) {
      turnIx++;
    ...
    await pushLayouts();
    await pushHost();
    // Auto-pick if the drawer stalls (ported: 15s default).
  ...
  async function selectWord(ix) {
    ...
    // Progressive hints at 40% / 65% / 80% elapsed (ported thresholds).
  ...
  return {
    ...
    async onPlayerJoin(player) {
      ...
      // Mid-game joiners can guess right away; they draw from the next game.
    ...
    onPlayerReconnect() {
      // Layout replay is automatic (platform snapshot); just refresh the TV.
      dirty = true;
    },
    ...
    async onInput(playerId, input) {
      ...
      if (playerId === drawerId && input.controlId === 'canvas') {
        if (input.action === 'clear') {
          ...
          // "colorIx|width|x,y;x,y;…" — coords quantized to 0..1000 by the phone.
      ...
      if (input.controlId === 'guess' && input.action === 'submit' && playerId !== drawerId && !guessed.has(playerId)) {
        ...
        if (guess === norm(word)) {
          ...
          await ctx.scores.add(playerId, earned); // session leaderboard
        ...
        // Wrong guess → TV feed; close guesses whisper back (ported).
