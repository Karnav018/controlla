// Word banks ported from ../server/services/wordBank.js (Dharmik's game)
// as an ESM module for the platform plugin. Selection uses ctx.random
// (platform contract: no Math.random in plugins).
...
export const ALL_WORDS = Array.from(
  new Set(Object.values(THEMED_WORDS).flat().map((w) => w.trim().toLowerCase().replace(/\s+/g, ' ')))
);

export { THEMED_WORDS };

export function wordList(theme) {
  if (theme === 'mix') return ALL_WORDS;
  const list = THEMED_WORDS[theme];
  return list && list.length > 0 ? list : THEMED_WORDS.classic;
}
